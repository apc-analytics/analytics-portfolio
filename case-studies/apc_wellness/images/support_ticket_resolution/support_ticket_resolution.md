...
2. **Speed** — How fast are tickets resolved overall, and does that vary
   ...
   resolve faster than low — worth confirming that's actually true, not
   just assumed.
...
channel comparisons, and whole-portfolio numbers are four genuinely
different grains — forcing them into one `group by` would mean answering
"which category drives the most volume" by mentally re-summing across
every priority/channel combination in a giant cross-tab. Four focused
outputs beat one that requires that kind of manual reconstruction.

### Anchoring to the data's own most recent point — again, but with a real subtlety

Same principle as the engagement case study: never compare against
`current_date`, since the ticket data is a static snapshot that drifts
further from "now" every day it isn't regenerated. The first attempt at
an anchor here used `max(resolved_at)` instead of `current_date` — an
improvement, but still wrong, for a reason worth spelling out: `resolved_at`
is only populated for tickets that have actually been resolved, and even
...
very recently that hasn't been resolved yet — exactly the opposite of
what a backlog-age calculation needs to be conservative about. `created_at`
is populated for every ticket regardless of status, so its maximum is the
correct anchor:
...
### A cast bug in the staging layer, caught by a bare `select *`

Before any of this analysis was possible, `select * from stg_apc_wellness__support_tickets`
itself failed with `invalid input syntax for type integer: "4.0"`. The
staging model cast `csat_rating` straight to `::int`, but the raw CSV
source stored the value as `"4.0"`, and Postgres's text-to-integer cast
rejects any string with a decimal point, even a whole-number one. Fixed
by casting through `numeric` first: `nullif(csat_rating, '')::numeric::int`.
A reminder that staging models are views — every query against one
re-runs its casting logic, so a bad cast anywhere in the chain surfaces
immediately, not just when that specific column is touched directly.

### Splitting "open" into two real, different states

An early pass treated any ticket without a `resolved_at` timestamp as
uniformly "open." The actual status field has three values — `Open`,
`Pending Customer Response`, `Closed` — and the first two mean genuinely
different things: `Open` is untouched and waiting on support, while
`Pending Customer Response` means support has already replied and is
waiting on the *customer*. Blending them into one "open" bucket would
hide exactly the distinction a backlog-health question needs. Fixed by
keying directly off `ticket_status` instead of inferring state from a
null check:
...
as one `with` block containing four independent CTEs — `overall_metrics`,
`category_metrics`, `priority_metrics`, `channel_metrics` — each checked
individually before being trusted. All four share the same structure,
differing only in the `group by` column:
...
— with one improvement the ad-hoc version couldn't make: the three
...
times. All 69 tests across the warehouse pass, including `unique`/`not_null`
...
Volume is fairly evenly spread across all five categories (27–39 tickets
each) and all four channels (37–45 each) — no single category or channel
...
under an actual check, not just an assumption left untested.

![Resolution time by channel](resolution_time_by_channel.png)

Channel tells a different, more operationally interesting story: **Phone
resolves fastest (2.3 days)**, followed by Social media (2.8) and Chat
(3.1), with **Email slowest (3.8 days)** — a real, if modest, spread
...
**App/device sync issue (2.6) is the clear laggard** — the one category
...
`Pending` — untouched tickets are aging noticeably worse than ones
already in progress. Two specific numbers stand out as the sharpest
...
**Program/feature question is the worst category (186.1 days)**. Two
cells buck the overall open > pending pattern — Critical priority and
Social media channel both show pending older than open — but with only
12–16 tickets in each of those specific cells, this reads as small-sample
noise rather than a real signal worth chasing as a bug.
...
1. **Investigate the Email backlog specifically.** At 229.8 days average
   ...
   analysis — worth understanding whether it's a staffing/routing issue
...
Priority-based routing is already working as intended — the monotonic
