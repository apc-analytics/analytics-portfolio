"""
APC Finance - raw data loader.

Loads the five generated CSVs (borrowers.csv, lenders.csv,
loan_applications.csv, loan_offers.csv, loans.csv) into db_portfolio's
raw_apc_finance schema.

Convention (matching raw_apc_wellness and raw_noaa_weather_us): every
column is loaded as TEXT, with no type casting here, which is dbt
staging's job. This keeps the loader simple and safely rerunnable: each
run drops and recreates every table fresh from the CSVs. DROP TABLE uses
CASCADE, since any staging/intermediate/mart views already built on top
of these raw tables need to be dropped along with them — dbt rebuilds
those automatically on the next run.
...
Usage:
    python load_apc_finance.py --data-dir "path\\to\\folder\\with\\the\\five\\csvs"
